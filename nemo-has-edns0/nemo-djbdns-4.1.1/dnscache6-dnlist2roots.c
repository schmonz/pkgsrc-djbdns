...
static unsigned int get_ips(void)
{
  ...
  static ip6_vector ips = IP6_VECTOR;
  register unsigned int i;
  stralloc *name;

  if (dns6_ns(&names, &fqdn) < 0) {
    log_fqdn_error();
    return 0;
  }
  ...
  for (i = 0; i < names.len; i++) {
    name = &names.va[i];
    if (dns6_ip6(&ips, name) < 0) {
      log_ip_error(name);
      continue;
    }
    if (!ip6_vector_cat(&servers, &ips)) die_nomem();
  }
  return servers.len;
}
...
int main(int argc, char **argv)
{
  ...
  while (match) {
    ...
    for (i = 0; i < servers.len; i++) {
      ...
      djbio_put(djbiofd_out, ip6str, ip6_fmt(&servers.va[i], ip6str));
      djbio_put(djbiofd_out, "]", 1);
    }
