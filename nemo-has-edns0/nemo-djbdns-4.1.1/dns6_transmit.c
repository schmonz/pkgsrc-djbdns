...
/* -1: fatal, 0: OK, 1: irrelevant */
static int irrelevant(struct dns6_transmit *dt, const byte_t *buf, unsigned int len)
{
...
static void dns6_transmit_packet_free(struct dns6_transmit *dt)
{
  if (!dt->packet) return;
...
static void dns6_transmit_query_free(struct dns6_transmit *dt)
{
  if (!dt->query) return;
  alloc_free(dt->query);
  dt->query = 0;
...
static void dns_socket_free(struct dns6_transmit *dt)
{
  if (dt->s < 0) return;
  close(dt->s);
  dt->s = -1;
...
void dns6_transmit_free(struct dns6_transmit *dt)
{
  dt->servers = 0;
  dns6_transmit_query_free(dt);
  dns_socket_free(dt);
  dns6_transmit_packet_free(dt);
}

static int random_bind(struct dns6_transmit *dt)
{
  unsigned int j;

  for (j = 0; j < 10; ++j) {
    if (socket6_bind(dt->s, &dt->localip, (uint16_t)(1025 + dns_random(64510))) == 0) {
      return 0;
    }
  }
  if (socket6_bind(dt->s, &dt->localip, 0) == 0) return 0;
  return -1;
}

static const unsigned int timeouts[4] = { 1, 3, 11, 45 };

static int this_udp(struct dns6_transmit *dt)
{
  struct taia now;
  ip6_address *ip;
...
int dns6_transmit_first_udp(struct dns6_transmit *dt)
{
  dt->curserver = 0;
  return this_udp(dt);
}

static int next_udp(struct dns6_transmit *dt)
{
  dt->curserver++;
  return this_udp(dt);
}

static int this_tcp(struct dns6_transmit *dt)
{
  struct taia now;
  ip6_address *ip;
...
int dns6_transmit_first_tcp(struct dns6_transmit *dt)
{
  dt->curserver = 0;
  return this_tcp(dt);
}

static int next_tcp(struct dns6_transmit *dt)
{
  dt->curserver++;
  return this_tcp(dt);
}

void dns6_transmit_init(struct dns6_transmit *dt)
{
  dt->s = -1;
  dt->query = 0;
  dt->packet = 0;
...
void dns6_transmit_io(struct dns6_transmit *dt, iopause_fd *x, struct taia *deadline)
{
  x->fd = dt->s;
...
int dns6_transmit_get(struct dns6_transmit *dt, iopause_fd *x, const struct taia *when)
{
  struct taia now;
  ...
  if (!x->revents) {
    ...
    errno = error_timeout;
    if (dt->tcpstate == 0) return next_udp(dt);
    return next_tcp(dt);
  }
