...
static const ip6_address *whichserver;
  ...
  while (i < num_total) {
    ...
    switch (dns_type_get(&type)) {
      ...
      case DNS_T_SOA:
        ...
        while (i < j) {
          ...
          if (records[i] >= pos_authority) {
            log6_rr_soa(whichserver, &t1, &t2, &t3, misc, ttl);
      ...
      case DNS_T_CNAME:
        ...
        if (!pos) return 0;
        dns_domain_lower(&t2);
        log6_rr_cname(whichserver, &t1, &t2, ttl);
      ...
      case DNS_T_PTR:
        ...
        while (i < j) {
          ...
          if (!pos) return 0;
          dns_domain_lower(&t2);
          log6_rr_ptr(whichserver, &t1, &t2, ttl);
          ioquery_save_data(t2.data, t2.len);
          i++;
        }
      ...
      case DNS_T_NS:
        ...
        while (i < j) {
          ...
          if (!pos) return 0;
          dns_domain_lower(&t2);
          log6_rr_ns(whichserver, &t1, &t2, ttl);
          ioquery_save_data(t2.data, t2.len);
          i++;
        }
      ...
      case DNS_T_MX:
        ...
        while (i < j) {
          ...
          if (!pos) return 0;
          dns_domain_lower(&t2);
          log6_rr_mx(whichserver, &t1, &t2, misc, ttl);
      ...
      case DNS_T_A:
        ...
        while (i < j) {
	      ...
	      log6_rr_a(whichserver, &t1, &ip4, ttl);
	    }
          }
          i++;
        }
      ...
      case DNS_T_AAAA:
        ...
        while (i < j) {
	      ...
	      log6_rr_aaaa(whichserver, &t1, &ip6, ttl);
            }
          }
          i++;
        }
      ...
      default:
        ...
        while (i < j) {
          ...
          log6_rr(whichserver, &t1, &type, buf + pos, data_len, ttl);
          i++;
        }
...
*/
  ...
  if (!rr_count) return DNS_RCODE_NOERROR;  /* safety */
...
*/
  if (flag_referral && !flag_soa) {
    if (dns_domain_equal(&referral, control) || !dns_domain_suffix(&referral, control)) {
      log6_lame(whichserver, control, &referral);
      lame6_servers_add(control, whichserver, soa_ttl);
      ip6_vector_remove(x->dt.servers, x->dt.curserver, 1);
...
void ioquery_havepacket(ioquery *x)
{
  ...
  if (rcode != DNS_RCODE_NOERROR) {
    log6_rcode(rcode, whichserver, &x->name, &x->type, soa_ttl);
  }
