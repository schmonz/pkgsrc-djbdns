...
void tcpclient_setup_socket(const ip4_address *ip_incoming)
{
  socket_tcp53 = socket4_tcp();
  if (socket_tcp53 < 0) die_tcpsocket();
  if (socket4_bind_reuse(socket_tcp53, ip_incoming, 53) < 0) {
    die_tcpsocket();
  }
}
...
{
  log4_drop_query(x->query_number, response_len, &x->ip, x->port, &x->id, x->loop);
  t_close(head, x);
}
...
void tcpclient_do_response(client *x)
{
  ...
  if (x->pos == x->len) {  /* finished */
    log4_query_done(x->query_number, response_len, &x->ip, x->port, &x->id, x->loop, response[3] & 15);
...
static void t_start_active(client *x)
{
  ...
  if (!packetquery(x->buf, x->len, &x->name, &x->type, &x->class, &x->id, &x->flag_edns0, &x->udp_size)) {
    log4_rejected_packet(&x->ip, x->port);
    tcpclient_drop_active(x);
    return;
  }

  log4_query(x->query_number, &x->ip, x->port, &x->id, &x->name, &x->type, x->flag_edns0, x->udp_size);
...
void tcpclient_start(void)
{
  ...
  x->tcp = socket4_accept(socket_tcp53, &x->ip, &x->port);
  if (x->tcp < 0) {
    t_free_connected(x);
    return;
  }
