...
static void copy(void *buf, unsigned int len)
{
...
static int build(stralloc *sa, const dns_domain *q, unsigned int flag_soa, const dns_id *id)
{
  ...
  if (!stralloc_catb(sa, "\204\0\0\0\0\1\0\0\0\0", 10)) die_nomem();
  copy(misc, 1);
  if ((misc[0] == '=' + 1) || (misc[0] == '*' + 1)) {
    --misc[0];
    copy(record_loc, 2);
    if (byte_diff(record_loc, 2, client_loc)) return 0;
  }
  if (misc[0] == '*') {
  ...
  dns_type_pack(&type, tmp);
  if (!stralloc_catb(sa, tmp, 2)) die_nomem();
...
static void do_axfr(const dns_id *id)
{
  ...
  key[2] = '4';
  key[3] = '\0';  /* padding */
  ip4_pack(&ip, key + KEY_PREFIX_LEN);
  if (!tdlookup_loc_setup(key, KEY_PREFIX_LEN+4)) die_cdbformat();
...
int main(int argc __UNUSED__, char **argv)
{
  ...
  for (;;) {
    ...
    if (dns_type_equal(&qtype, dns_t_axfr) || dns_type_equal(&qtype, dns_t_ixfr)) {
      ...
      if (!respond4(&zone, &qtype, &ip, DNS_UDP_SIZE_DEFAULT, 0)) die_outside();
