...
static unsigned int server6_doit(ip6_address *ip, uint16_t port, unsigned int *udp_size)
{
...
*/
  ...
  if ((header[2] & 126) == 0x24) {  /* NOTIFY + AA */
    if (type == DNS_T_SOA) {
      ...
      qlog6(ip, port, &qid, &qname, &qtype, " N ");
      return 1;
    }
  }
...
*/
  ...
  *udp_size = udp_payload_size;
  ...
  r = respond6(&qname, &qtype, ip, *udp_size, flag_edns0);
  switch (r) {
  ...
  qlog6(ip, port, &qid, &qname, &qtype, result);
  return r;

NOTIMP:
  response_notimp();
  qlog6(ip, port, &qid, &qname, &qtype, " I ");
  return 1;

FORMERR:
  response_formerr();
  qlog6(ip, port, &qid, &qname, &qtype, " F ");
  return 1;
/*
REFUSE:
  response_refused();
  qlog6(ip, port, &qid, &qname, &qtype, " R ");
  return 1;
*/
BADVERS:
  response_opt_error(DNS_RCODE_BADVERS);
  qlog6(ip, port, &qid, &qname, &qtype, " V ");
  return 1;

NOQ:
  dns_id_unpack(&qid, "\0\0");
  qlog6(ip, port, &qid, dns_d_empty, dns_t_nil, " / ");
  return 0;
}
...
int main(int argc __UNUSED__, char **argv)
{
  ...
  if (socket6_bind_reuse(udp53, &ip, 53) < 0) die_bind("UDP");
  ...
  for (;;) {
    r = socket6_recv(udp53, buf, sizeof(buf), &ip, &port);
    if (r < 0) continue;
    len = (unsigned int)r;
    if (!server6_doit(&ip, port, &udp_size)) continue;
    if (response_len > udp_size) response_tc();
    response_send6(udp53, &ip, port);
