...
static mxip6_vector out = MXIP6_VECTOR;
static char ip6str[IP6_FMT];
...
int main(int argc, char **argv)
{
  ...
  while (*argv) {
    ...
    stralloc_lower(&fqdn);
    if (flag_ipv6) {
      if (dns6_mxip6(&out, &fqdn) < 0) die_rr_query("MX", *argv);
    }
    else {
      if (dns4_mxip6(&out, &fqdn) < 0) die_rr_query("MX", *argv);
    }

    mxip6_vector_sort(&out);
    if (!out.len) {
	...
	die_internal();
      ...
      for (i = 0; i < out.len; i++) {
        ...
        if (!stralloc_catb(&buffer, ip6str, ip6_fmt(&cur->ip, ip6str))) die_nomem();
        djbio_putsa(djbiofd_out, &buffer);
        djbio_puteol(djbiofd_out);
      }
    }
