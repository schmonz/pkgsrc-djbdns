...
static int resolve4(const dns_domain *q, dns_type *qtype, ip4_address *ip)
{
  ...
  if (dns4_transmit_start(&tx4, &servers, 0, q, qtype, null_ip4) < 0) return -1;
  for (;;) {
    taia_now(&stamp);
...
static void query4_add(const dns_domain *owner, const dns_type *type)
{
  ...
  for (i = 0; i < ns.len; ++i) {
    if (dns_domain_suffix(owner, &ns.va[i].owner)) {
      for (j = 0; j < address4.len; ++j) {
        if (dns_domain_equal(&ns.va[i].ns, &address4.va[j].name)) {
          qt4_add(owner, type, &ns.va[i].owner, &address4.va[j].ip);
        }
      }
    }
  }
...
static void ns4_add(const dns_domain *owner, const dns_domain *server)
{
  ...
  for (i = 0; i < query.len; ++i) {
    if (dns_domain_suffix(&query.va[i].owner, owner)) {
      for (j = 0; j < address4.len; ++j) {
        if (dns_domain_equal(server, &address4.va[j].name)) {
          qt4_add(&query.va[i].owner, &query.va[i].type, owner, &address4.va[j].ip);
        }
      }
    }
  }
...
static void parse_packet4(const byte_t *buf, unsigned int len, const dns_domain *d, const dns_type *dtype, const dns_domain *control)
{
  ip4_address misc4;
  ip6_address misc6;
  dns_type type;
...
{
  static ip4_vector out4 = IP4_VECTOR;
  ...
  query4_add(&qname, &type);
  ns4_add(dns_d_empty, dns_d_empty);
  while (*++argv) {
    if (!stralloc_copys(&udn, *argv)) die_nomem();
    if (dns4_ip4_qualify(&out4, &fqdn, &udn) < 0) {
      if (errno == error_nomem) die_nomem();
      if (errno == error_proto) die_parse("bad root ip", *argv);
      die_internal();
    }
    for (i = 0; i < out4.len; i++) {
      address4_add(dns_d_empty, &out4.va[i]);
    }
  }
  for (i = 0; i < qt4.len; ++i) {
    if (!dns_domain_copy(&qname, &qt4.va[i].owner)) die_nomem();
    if (!dns_domain_copy(&control, &qt4.va[i].control)) die_nomem();
    if (!dns_domain_suffix(&qname, &control)) continue;
    dns_type_copy(&type, &qt4.va[i].type);
    ip4_copy(&ip4, &qt4.va[i].ip);
    ...
    if (!stralloc_append(&querystr, ":")) die_nomem();
    if (!stralloc_catb(&querystr, ipstr, ip4_fmt(&ip4, ipstr))) die_nomem();
    if (!stralloc_append(&querystr, ":")) die_nomem();
    ...
    if (resolve4(&qname, &type, &ip4) < 0) {
      ...
      parse_packet4(tx4.packet, tx4.packetlen, &qname, &type, &control);
    }

    if (dns_domain_equal(&qname, dns_d_ip4_localhost)) {
      djbio_putsa(djbiofd_out, &querystr);
      djbio_puts(djbiofd_out, "ALERT:some caches do not handle localhost internally\n");
      address4_add(&qname, localhost_ip4);
    }
    if (dd4(&qname, dns_d_empty, &ip4) == 4) {
      djbio_putsa(djbiofd_out, &querystr);
      djbio_puts(djbiofd_out, "ALERT:some caches do not handle IPv4 addresses internally\n");
      address4_add(&qname, &ip4);
    }
