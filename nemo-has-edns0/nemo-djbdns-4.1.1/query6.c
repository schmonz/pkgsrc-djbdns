...
{
  log_glueless_aaaa(name, control);
  cache_mark(dns_t_aaaa, name, CACHE_NXDOMAIN, 0);
}

static unsigned int do_query_servers(query *z)
{
  dns_sortip6(&z->servers);
...
static unsigned int ns_loop_exists(query *z, dns_domain *name)
{
  ...
  t = z->next;
  while (t) {
    if (dns_domain_equal(name, &t->name) && dns_type_equal(dns_t_aaaa, &t->type)) return 1;
    t = t->next;
  }
  return 0;
}
...
/*
  look for all NS with AAAA record
...
static query_ns_t do_query_ns(query *z)
{
  ...
  for (i = 0; i < z->ns.len; i++) {
    ...
    if (cache_test_rr(dns_t_cname, qname) == CACHE_HIT) {  /* cname prohibited in ns - RFC 2181 s10.3 */
      ...
      cache_mark(dns_t_aaaa, qname, CACHE_SERVFAIL, 0);
      ns_errors++;
      continue;
    }
    ...
    status = cache_get_rr_aaaa(qname, &servers);
    debug_putuint("do_query_ns:cache_get_rr_aaaa", status);
    if (status == CACHE_HIT) {
      if (!ip6_vector_cat(&z->servers, &servers)) die_nomem();
    ...
    if (status == CACHE_MISS) {
      ...
      query_start(z->client, dns_t_aaaa, qname, 0);
      debug_putquery("do_query_ns:query_start", qname, dns_t_aaaa);
    ...
    if (status == CACHE_EXPIRED) {
      ...
      cache_mark(dns_t_aaaa, qname, CACHE_MISS, 0);
      return R_QUERY_NS_RETRY;
    }
  ...
  lame6_servers_prune(qcontrol, &z->servers);
