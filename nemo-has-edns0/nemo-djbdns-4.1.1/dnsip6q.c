...
static ip6_vector out = IP6_VECTOR;
static char str[IP6_FMT];
...
int main(int argc, char **argv)
{
  ...
  while (*argv) {
    ...
    stralloc_lower(&fqdn);
    if (flag_ipv6) {
      if (dns6_ip6_qualify(&out, &fqdn, &in) < 0) die_not_found("IP address", *argv);
    }
    else {
      if (dns4_ip6_qualify(&out, &fqdn, &in) < 0) die_not_found("IP address", *argv);
    }

    djbio_putsa(djbiofd_out, &fqdn);
    djbio_put(djbiofd_out, " ", 1);
    for (i = 0; i < out.len; i++) {
      djbio_put(djbiofd_out, str, ip6_fmt(&out.va[i], str));
      djbio_put(djbiofd_out, " ", 1);
    }
