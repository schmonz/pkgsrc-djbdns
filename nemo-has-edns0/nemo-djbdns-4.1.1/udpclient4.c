...
{
  socket_udp53 = socket4_udp();
  if (socket_udp53 < 0) die_udpsocket();
  if (socket4_bind_reuse(socket_udp53, ip_incoming, 53) < 0) die_udpsocket();
}
...
static void u_drop(client **head, client *x)
{
  ...
  response_id(&x->id);
  response_servfail();
  response_send4(socket_udp53, &x->ip, x->port);
...
void udpclient_start(void)
{
  ...
  len = socket4_recv(socket_udp53, buf, sizeof(buf), &ip, &port);
  if (len < 0) return;
  ...
  log4_query(x->query_number, &ip, port, &id, &d, &type, flag_edns0, udp_size);
