...
const char USAGE[] = "acct logacct /axfrdns6 /tinydns myip";
...
int main(int argc, char **argv)
{
  ...
  outs(" tcpserver6 -vDRHL -l 0 -x tcp.cdb -- ");
  outs(auto_prefix);
  outs("/bin/axfrdns6\n");
  finish();
  perm(0755);
  ...
  start("Makefile");
  outs("tcp.cdb: tcp\n");
  outs("\ttcprules -6 tcp.cdb tcp.tmp < tcp\n");
  finish();
  perm(0644);

  start("tcp");
  outs("# sample line:  [::1]:allow, AXFR=\"example.com/1.0.0.ip6.arpa\"\n");
  outs(":deny\n");
  finish();
  perm(0644);
