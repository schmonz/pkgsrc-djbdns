...
const char INFO[] = "dnscache6-conf: ";
...
int main(int argc, char **argv)
{
  ...
  seed_addtime();
  outs("data: @\n");
  outs("\tcat @ | dnscache6-iplist2data \".\" > data\n");
  seed_addtime();
  finish();
  seed_addtime();
  perm(0755);
  ...
  outs("#!/bin/sh\n");
  seed_addtime();
  outs("dnsip6 `dnsqr ns . | awk '/answer:/ {print $5;}' | sort` | sed -e 's/ //' | grep ':' > root/servers/@\n");
  seed_addtime();
  finish();
  seed_addtime();
  perm(0755);
  ...
  outs("tcp tpi_cots_ord v inet6 tcp /dev/tcp -\n");
  outs("udp tpi_clts v inet6 udp /dev/udp -\n");
  finish();
  perm(0645);
  umask(000);
