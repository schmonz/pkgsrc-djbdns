...
static void output_ip6_results(const stralloc *server, const ip6_address *result)
{
  char result_ipstr[IP6_FMT];
  unsigned int result_ipstr_len;
  if (flag_quiet) return;
  result_ipstr_len = ip6_fmt(result, result_ipstr);
  djbio_putsa(djbiofd_out, server);
  djbio_put(djbiofd_out, ": ", 2);
...
static ip6_vector ip6_results = IP6_VECTOR;
...
static void check_name(void)
{
  ...
  for (i = 0; i < base_list.len; i++) {
    ...
    if (flag_rr_txt) {
      ...
      if (dns6_ip6(&ip6_results, &fqdn) < 0) die_dns_query();
      if (ip6_results.len) {
        if (flag_exit_on_first && count) return;
        output_ip6_results(current_server, &ip6_results.va[0]);
        count++;
      }
    }
  }
...
int main(int argc, char **argv)
{
  ...
  x = *argv;
  if (!*x) die_usage1("FQDN not supplied");
